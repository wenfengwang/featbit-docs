...
您可能已经在自己的基础架构中运行了 MongoDB、Redis、Kafka、Zookeeper 和/或 ClickHouse 服务，并且您希望使用自己的服务运行 FeatBit，这是完全可能且易于做到的。本文档将说明如何使用 docker compose 来做到这一点，它也可以很容易地转换为 Kubernetes。

本文档将向您展示如何运行以下 FeatBit 服务：

* **UI：** 为用户提供可视化的 UI 界面，用于管理和发布功能标志、细分和实验等。
...
<Callout type="info">建议 Windows 用户使用 PowerShell 运行以下命令。</Callout>
...
* 如果您正在进行快速测试，我们已创建了一个 **docker-compose-infra.yaml** 文件，您可以使用它来安装 FeatBit 的基础服务（Redis、MongoDB、Kafka、Zookeeper 和 ClickHouse），以便顺利进行下一步。如果您想使用自己的基础服务，请跳过此步骤，运行以下命令启动这些服务。

```sh
docker compose -f docker-compose-infra.yml up -d
```

* **如果您已经执行了上一步，请跳过此步骤**。我们还创建了另一个 docker compose 文件 **docker-compose-clickhouse.yml** 以仅安装 ClickHouse，以防您决定使用自己的基础设施但您没有 ClickHouse。在这种情况下，在以下路径设置 Zookeeper 配置： **featbit/infra/clickhouse/single_node/config.xml** 
...
*   在您喜欢的编辑器中打开 _**docker-compose-partial.yml**_，您会看到以下配置。假设基础设施服务（Redis、MongoDB、Kafka、ClickHouse）与我们将在本文档中安装的 FeatBit 服务运行在同一个网络中。
    &#x20; &#x20;
...
ui:   
  ...
  environment:
    ...
    - DEMO_URL=https://featbit-samples.vercel.app # 保持原样，这是为了教程
...
evaluation-server:
  ...
  environment:
      - MongoDb__ConnectionString=[REPLACE_WITH_MONGODB_CONNECTION_STRING] # 同 api-server
      - MongoDb__Database=[REPLACE_WITH_MONGODB_NAME] # 同 api-server
      - Kafka__BootstrapServers=[REPLACE_WITH_KAFKA_CONNECTION_STRING] # 同 api-server
...
da-server:
  ...
  environment:
    ...
    KAFKA_SECURITY_PROTOCOL: [REPLACE_WITH_KAFKA_SECURITY_PROTOCOL] # 其中之一 PLAINTEXT, SASL_PLAINTEXT, SASL_SSL, SSL
    # 如果 Kafka 安全协议是 SASL_PLAINTEXT, SASL_SSL，请设置以下 3 个变量
...
我们提供了一个 [mongodb-seed-script](https://github.com/featbit/featbit/blob/main/infra/mongodb/docker-entrypoint-initdb.d/init.js) 来初始化我们的 MongoDB 数据库。此脚本假定数据库名称为“featbit”（您可以更改），它应该与 MongoDb__Database 环境变量相同。
...
```
* 启动所有容器后，转到 FeatBit 的门户 [http://localhost:8081](http://localhost:8081/) 并使用默认凭据登录。
  * 用户名： **test@featbit.com**
  * 密码： **123456**

## 更新

运行以下命令以更新到最新的 FeatBit 镜像：
