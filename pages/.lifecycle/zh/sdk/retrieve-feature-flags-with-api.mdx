...
本文档提供了一份全面的指南，介绍了在目标 SDK 不可用时，如何直接从服务器检索功能标志。

评估服务器提供了两个 API，允许您轻松地检索特定于所需环境的客户端和服务器端平台的功能标志。

客户端 SDK 和服务器端 SDK 的区别如下：

* 客户端 SDK 适用于单个用户，而服务器端 SDK 适用于多个用户
* 客户端 SDK 依赖于评估服务器来评估所有标志，而服务器端 SDK 在本地评估标志

<Callout type="info" emoji="ℹ️">
请注意，您需要用适合您环境的主机 URL替换 API 请求中的`{Host}`。 `{Host}` 可以是评估服务器主机或 FeatBit Agent 主机，两者都提供相同的 API。
</Callout>

## 客户端

对于客户端请求，请在`Authorization`标头中提供客户端环境密钥。
...
对于服务器端请求，请在`Authorization`标头中提供服务器环境密钥。
